...
gagedat <- read_csv(here("data", "Intellus_2023_WT_All.csv"))


# Read the csv data and clean the column header names
current_locs <- (gagedat) %>%
  clean_names() %>%
...
output_file <- here("output", paste0("current_samples_collected_2023.html"))
# Save the map as an html with the sampling year in the title
saveWidget(circle_map, file = output_file, selfcontained = TRUE)
